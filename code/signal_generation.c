...
void signal_generation_init(signal_config* config) 
{
    ...
    BSP_DAC_Start_DMA(&hdac, DAC1_CHANNEL_1, config->channel_1_sine_val, Ns, DAC_ALIGN_12B_R);
    BSP_DAC_Start_DMA(&hdac, DAC1_CHANNEL_2, config->channel_2_sine_val, Ns, DAC_ALIGN_12B_R);
...
void get_channel_1_sine(uint32_t* channel_1_sine_val, signal_config* config) 
{
    for (int i = 0; i < Ns; i++) 
    {
        channel_1_sine_val[i] = ((sin(i * 2 * PI / Ns) + 1) * (config->res / 2));
...
void get_channel_2_sine(uint32_t* channel_2_sine_val, signal_config* config) 
{
    for (int i = 0; i < Ns; i++) 
    {
        channel_2_sine_val[i] = ((sin(i * 2 * PI / Ns) + 1) * (config->res / 2));
...
void set_clock_tim2(signal_config* config) 
{
    ...
    config->psc = (config->fclock / Ns) / (config->freq_signal_1 * (config->period + 1)) - 1;
...
void set_clock_tim4(signal_config* config) 
{
    ...
    config->psc = (config->fclock / Ns) / (config->freq_signal_2 * (config->period + 1)) - 1;

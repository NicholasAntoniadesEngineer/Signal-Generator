...
void signal_generation_init(signal_state* state) 
{
    ...
    BSP_DAC_Start_DMA(&hdac, DAC1_CHANNEL_1, state->channel_1_sine_val, state->Ns, DAC_ALIGN_12B_R);
    BSP_DAC_Start_DMA(&hdac, DAC1_CHANNEL_2, state->channel_2_sine_val, state->Ns, DAC_ALIGN_12B_R);
...
void set_clock_tim2(signal_state* state) 
{
    ...
    htim2.Init.Period = state->period;
    htim2.Init.Prescaler = state->psc;
...
void set_clock_tim4(signal_state* state) 
{
    ...
    htim4.Init.Period = state->period;
    htim4.Init.Prescaler = state->psc;
